import { z } from 'zod';
import { LOG_LEVELS, DEFAULT_REQUEST_TIMEOUT } from './constants.js';
...
const configSchema = z.object({
    ...
    logLevel: z.enum(LOG_LEVELS).default('error'),
    maxRetries: z.number().int().positive().default(3),
    requestTimeout: z.number().int().positive().default(DEFAULT_REQUEST_TIMEOUT),
});
...
export function loadConfig(): Config {
    ...
    return configSchema.parse({
        ...
        maxRetries: process.env.MAX_RETRIES ? Number(process.env.MAX_RETRIES) : 2,
        requestTimeout: process.env.REQUEST_TIMEOUT ? Number(process.env.REQUEST_TIMEOUT) : DEFAULT_REQUEST_TIMEOUT,
    });
}
